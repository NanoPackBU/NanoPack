...
if isWindows:
    pathChar = "\\"
working_dir = os.getcwd() + pathChar
    ...
    for rect in rects:
        ...
        if (rect[2] - rect[0]) * (rect[3] - rect[1]) > 25000:
            ...
            img_copy = cv2.rectangle(img_copy, (rect[0], rect[1]), (rect[2], rect[3]),
                                     (255, 0, 0), 3)
    # cv2.imshow("img final", img_copy)
    # cv2.waitKey(0)
    return img_copy, final_rects


def crop_to_rectangle(img, rect):
    # print(img.shape)
    # print(rect)
    l = rect[1]
    r = rect[3]
...
def crop_chips(chips_img, rects, count=0, out_folder="", write=True):
    img = chips_img
    for rect in rects:
        cropped = crop_to_rectangle(img, rect)
        num = crop_num_rec(cropped)
        if write:
            name = working_dir + "chip_outputs" + pathChar + "chip_" + str(count) + ".png"
            num_name = working_dir + out_folder + pathChar + "num_" + str(count) + ".png"
