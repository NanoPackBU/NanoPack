...
def ps_imagewarpCalibration():
    ...
    image_file = cbook.get_sample_data('/Users/paulstephenmalta/Documents/GitHub/NanoView_G33/dev/machine_learning/yoloML/t1.jpg')
...
def fig_to_cv2(fig):
    ...
    return img
def stich_image(list_of_image,DEBUG = False):
    row_num = len(list_of_image[0])
    col_num = len(list_of_image)
    img = list_of_image[0][0]
    if(DEBUG):cv2.imshow("haha",img)
    if(DEBUG):cv2.waitKey(1000)
    h,w,_ = img.shape
...
def Intersecting(x1, y1, w1, h1, x2, y2, w2, h2):
    Xs = (((x1 > x2) and (x1 < (x2 + w2))) or ((x2 > x1) and (x2 < (x1 + w1))))
    Ys = (((y1 > y2) and (y1 < (y2 + h2))) or ((y2 > y1) and (y2 < (y1 + h1))))
    return (Xs and Ys)
def makeCanvas(w, h, src=""):  # working
    if src == "":
...
def randomGraidentlight(img, amt=3):
    h, w, d = img.shape
    gradent = np.zeros((h, w, d))
    ...
    gradent = gradent.astype("uint8")

    Rotmat = cv2.getRotationMatrix2D((h / 2, w / 2), angle, 1.0)
    rotated = cv2.warpAffine(gradent, Rotmat, (h, w))

    biggerGradient = cv2.resize(rotated, [math.ceil(1.5 * h), math.ceil(1.5 * w)], interpolation=cv2.INTER_AREA)
    croped = biggerGradient[math.ceil(h / 4):h, math.ceil(w / 4):w, :]
    cropedToscale = cv2.resize(croped, [w, h], interpolation=cv2.INTER_AREA)
    factor = amt / 10
    floatVal = cropedToscale / 255.0
...
def randomPass(img):
    ...
    blured = blur(img)
    amt = rd.randint(0, 5)
    add_noisey = add_noise(blured, amt)
    amt = rd.randint(0, 10)
    lit_wierd = randomGraidentlight(add_noisey, amt)
    return lit_wierd

def formatImage(img=None,path=None,dest=None,dims=(512,512)):
    if (img != None):
        pass
    if (img == None and (path!=None and dest!=None)):
        img = cv2.imread(path)
    if (img == None and path == None and dest == None):
        print("No Args")
        return
    ...
    if ((path!=None and dest!=None)):cv2.imwrite(dest, adjusted)
    return adjusted
...
def formate_edge_guess(q1,q2,q3,q4):
    ...
    # --- 6= #|
    # --- 8= #^_
    if (q1 and q2 and q3 and q4): return 0
    if (not q1 and not q2 and not q3 and not q4): return 5
    if (q1 and not q2 and not q3 and not q4): return 9
    if (q1 and q2 and not q3 and not q4): return 8
    if (q2 and not q1 and not q3 and not q4): return 7
    if (q2 and q3 and not q1 and not q4): return 4
    if (q3 and not q1 and not q2 and not q4): return 1
    if (q3 and q4 and not q1 and not q2): return 2
    if (q4 and not q1 and not q2 and not q3): return 3
